(function(root){
	...
	BarChart.prototype.select = function(r){
		for(var b = 0; b < this.nbins; b++){
			...
			if(this.bins[b].selected) document.getElementById(this.bins[b].id).classList.remove('deselected');
			else document.getElementById(this.bins[b].id).classList.add('deselected');
		}
		return this;
	}
	BarChart.prototype.toggleBin = function(b){
		this.bins[b].selected = !this.bins[b].selected;
		document.getElementById('#'+this.bins[b].id+'').classList.toggle('deselected');
		return this;
	}
	...
	BarChart.prototype.draw = function(){
		...
		var id = this.target.getAttribute('id').replace(/^([^\s]+).*$/,function(m,p1){return p1;});
		...
		if(this.nbins > 0){
			this.target.classList.add('barchart');
			this.target.style['margin-bottom'] = '2.25em';

			if(typeof this.lineheight!=="number"){
				appendHTML(this.target,'<table class="remove"><tr><td><span class="label">label</span></td></tr></table>');
				styles = window.getComputedStyle(this.target.querySelector('table .label'));
				this.lineheight = parseInt(styles.lineHeight)+parseInt(styles.paddingBottom)+parseInt(styles.paddingTop);
				this.target.querySelectorAll('.remove').forEach((el)=>{ el.remove() });
			}
			// Set the height of the graph
			if(!this.height) this.height = this.target.getAttribute('offsetHeight') || parseInt(this.target.style.height) || 200;
			...
			if(!this.drawn) appendHTML(this.target,'<div class="grid" style="height:100%;"></div><table style="height:100%"><tr style="vertical-align:bottom;"></tr></table><div style="clear:both;"></div>');
			...
			appendHTML(this.target.querySelector('.grid'),output);

			var maketable = (this.target.querySelectorAll('table td').length == 0);
			output = "";
			...
			for(var b = 0; b < this.nbins; b++){
				...
				if(maketable){
					...
					p = document.getElementById(idbar);
console.log(p,idbar);
					if(p){
						// Update top antibar
						p.querySelectorAll('.antibar')[0].style.height = htop.toFixed(4)+'%';
						// Update bottom antibar
						p.querySelectorAll('.antibar')[1].style.height = hbot.toFixed(4)+'%';

						// Update each series bar height
						var bars = p.querySelectorAll('.bar');
						for(var br = 0; br < bars.length; br++){
							s = parseInt(bars[br].getAttribute('data-index-series'));
							var v = 0;
							...
							hbb = hb*(Math.abs(this.bins[b].values[s])/v);
							if(isNaN(hbb)) hbb = 0;
							bars[br].style.height = hbb.toFixed(4)+'%';
							bars[br].setAttribute('title',key+': '+(this.attr.units || "")+this.formatNumber(this.bins[b].values[s]));
						}
						p.querySelectorAll('.label').forEach((el)=>{appendHTML(el,this.attr.formatX.call(this,key));});
					}
				}
			}
			...
			if(maketable){
				// Add the table cells
				appendHTML(this.target.querySelector(' table tr'),output);
				addEv('mouseleave',this.target,{me:this},function(e){ if(e.data){ e.data.me.trigger("mouseleave",{event:e}); } })
				addEv('mouseover',this.target,{me:this},function(e){ e.data.me.trigger("mouseover",{event:e}); })
			}

			// Attach the events
			if(!this.drawn){
				var bars = this.target.querySelectorAll('.bar');
				var b;
				for(b = 0; b < bars.length; b++){
					addEv('focus',bars[b],{me:this,parent:this.attr.parent},function(e){ e.currentTarget = e.currentTarget.parentNode; e.data.me.trigger("barover",{event:e,bin:e.currentTarget.getAttribute('data-index'),series:e.currentTarget.getAttribute('data-index-series')}); });
					addEv('click',bars[b].parentNode,{me:this,parent:this.attr.parent},function(e){ e.preventDefault(); e.data.me.trigger("barclick",{event:e,bin:e.currentTarget.getAttribute('data-index')}); });
					addEv('mouseover',bars[b].parentNode,{me:this,parent:this.attr.parent},function(e){ e.data.me.trigger("barover",{event:e,bin:e.currentTarget.getAttribute('data-index')}); });
				}
			}
			this.drawn = true;
		}
	...
	OI.barchart = function(target,bins,attr){ return new BarChart(target,bins,attr); }
	root.OI = OI||root.OI||{};
